<header class="header-index-page">
                            ...
                            @foreach($all_item_card as $item)
                                @if($item->user_id == auth()->user()->id)
                                    ...
                                    <li>
                                        ...
                                        <span class="name-product-view-card"><a
                                                href="{{route('product.show' , ['slug'=>$item->products->slug])}}">{{$item->products->name}}</a></span>
                                        <span
                                            class="number-product-view-card">تعداد <span>{{$item->number}}</span></span>
                                        <i @click="deleteProductCard('{{$item->id}}')"
                                           class="fas fa-trash-alt delete-product-card" title="حذف محصول"></i>
                                    </li>
                                @endif
                            @endforeach
                        </ul>
    ...
    <div class="group-icon-header group-input-search">
        ...
        <input v-model="text_search_index_page" @keyup="searchIndexPage" type="text" class="input-search-index-page"
