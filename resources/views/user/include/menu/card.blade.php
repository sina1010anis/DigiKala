...
<div class="group-save-product" style="position: relative">
            ...
            <span class="view-list-buy" style="padding: 10px;box-sizing: border-box;background-color: unset">
                ...
                @foreach($all_item_card as $save_product)
                        <a class="view-product-save fl-right" href="{{route('product.show' , ['slug'=>$save_product->products->slug])}}">
                            ...
                            <p align="center" class="set-font f-12 color-b-600">قیمت : {{$save_product->total_price}}</p>
    ...
    <button class="set-font f-12 color-b-600 btn-new-address" @click="showPageNewAddress">پرداخت با مبلغ : {{$price}}</button>
</div>
@else
